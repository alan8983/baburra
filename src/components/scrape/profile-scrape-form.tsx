...
export function ProfileScrapeForm({ onJobCreated }: ProfileScrapeFormProps) {
  ...
  const isValidUrl = useMemo(() => {
    return url.trim() ? YOUTUBE_CHANNEL_PATTERN.test(url.trim()) : null;
  }, [url]);

  const isYouTube = useMemo(() => YOUTUBE_CHANNEL_PATTERN.test(url.trim()), [url]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              {isYouTube && <Youtube className="h-5 w-5 shrink-0 text-red-500" />}
              <Input
                id="scrape-url"
                type="url"
