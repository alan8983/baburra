...
**v0.2.0 完成度: ~70%** (Phase 12b + 17 已完成，Phase 15-lite + 16 進行中)
...
| A/B 測試框架 | 2026-03-01 | ab-test.ts + ab-experiment.repository + /api/ab/events + middleware 50/50 分流 + migration 017 |
| Welcome 頁面 (Variant B) | 2026-03-01 | /welcome 2 步驟預註冊體驗 (產品介紹 → 匯入)，匿名 session + 資料保留至註冊 |
| 統一錯誤處理 | 2026-03-01 | fetch-error.ts (ApiError class) + parse-error.ts + 全 API routes 結構化錯誤回應 + hooks 整合 |
| 驗證模組擴充 | 2026-03-01 | validation.ts 統一 Zod schemas (posts, KOLs, stocks, bookmarks, drafts, profile, AI) |
| DB 原子操作 | 2026-03-01 | create_post_atomic() + refund_ai_quota() + ai_quota 非負 CHECK + consume_ai_quota JSONB 修正 (migration 013-016, 018) |
| KOL/Stock 統計 View | 2026-03-01 | kol_stats + stock_stats materialized views (migration 016) |
| 三角標記圖表元件 | 2026-03-01 | triangle-markers-primitive.ts — K 線圖情緒三角形標記 (▲/▼/●) |
| PriceChangeBadge 元件 | 2026-03-01 | 可複用漲跌幅 badge，整合色盤系統 |
...
| YouTube 日期修正 | 2026-03-14 | 改用 YouTube Data API v3 取得影片原始發布日期，修正先前顯示爬取日期 + 時間鎖定 8:00 AM 問題 |
...
2026-03-01  ██████████ A/B 測試框架 + Welcome 頁面 + 統一錯誤處理 + DB 原子操作 + Hook 測試套件
```
...
CREATE TABLE stocks (
  ...
  logo_url TEXT,
  market TEXT DEFAULT 'US',                -- US | TW | HK | CRYPTO (v1.0.0)
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
- [x] URL 自動匯入 (Twitter/X only — 使用免費 oEmbed API) ✅ (twitter.extractor.ts 已實作)
- [x] RWD 響應式設計 ✅ (sidebar collapse + mobile-nav.tsx 已實作)
- [x] 書籤管理功能 ✅ (2026-02-12, 提前至 v0.1.0 完成)
- [ ] ~~AI 文章摘要~~ → **延後至 v1.0.0**
- [ ] ~~編輯建議系統~~ → **延後至 v1.0.0** (DB 表已存在，app 邏輯待實作)
- [x] ~~**AI 分析快取層 (Phase 15)**~~ → **已移除** (Phase 12b 的 `UNIQUE(source_url)` 重複檢查已涵蓋快取需求)
- [ ] **AI 模型版本追蹤 + 重新分析 (Phase 15-lite)**
  - `posts` 表新增 `ai_model_version` 欄位，記錄分析時使用的模型版本
  - 當 `AI_SENTIMENT_MODEL` env var 更新時，舊版本的 posts 標記為「可重新分析」
  - 提供「重新分析」按鈕 (單篇 + 批量)，以新模型重跑 sentiment/tickers/arguments
  - 為未來 Argument Analysis 重新啟用做準備 — 新模型可能產出更好的論點提取結果
  - 不含：快取表、自動失效、通知機制
...
- [x] **KOL Profile 爬取 + 訂閱監控 (Phase 12b)** ✅ (Waves 1-4 完成)
  - 用戶輸入 KOL 的 Profile URL (YouTube 頻道)，系統自動爬取歷史內容 (最多 50 影片)
  - Posts 維持全域共享 (`UNIQUE(source_url)`)：爬取一次，所有用戶受益
  - 新表：`kol_sources` (平台身份+爬取狀態)、`kol_subscriptions` (用戶訂閱)、`scrape_jobs` (背景任務佇列)
  - `ProfileExtractor` 抽象類別 + `YouTubeChannelExtractor` (YouTube Data API v3, 免費)
  - `profile-scrape.service.ts` — 爬取調度器，reuse existing import pipeline with `quotaExempt: true`
  - 背景處理：Vercel Cron Functions (`/api/cron/process-jobs` 每 5 分鐘、`/api/cron/monitor-subscriptions` 每 6 小時)
  - 可恢復批次處理：`discovered_urls` JSONB + `processed_urls` 計數器，每次 3-5 URLs (60s timeout safe)
  - 訂閱系統：Follow/Unfollow 按鈕、每日自動檢查新內容、自動啟用/停用監控
  - 訂閱額度：Free 10 KOL / Paid 50 KOL（統一概念，取代舊 "5 KOL" 限制）
  - 歷史資料擴展：每位新訂閱者觸發 +10 篇更早文章爬取，上限 200 影片 / 500 推文 / 500 貼文
  - 平台路線圖：YouTube (免費) → Twitter/X (pay-as-you-go) → Instagram+Facebook (第三方 Bright Data/Apify)
  - AI 模型可配置 (`AI_SENTIMENT_MODEL` env var)，預設 Gemini Flash，支援未來 A/B 測試
  - 效能策略 (Option A+B)：首批 3-5 篇即時處理 → Cron 批次升級至 `Promise.allSettled()` 平行 8-10 篇 → 進度 UI 每 5s 輪詢 → 50 影片約 25-45 分鐘完成
  - 10 個子階段 (12b-1 ~ 12b-9)，以 4 波平行 Agent 執行 + 2 個延後階段
- [x] **行銷首頁 (Phase 17)** ✅ (marketing route group 已實作)
  ...
  - 定價方案：Free (10 KOL 追蹤 / 20 AI 分析/月) vs Pro (50 KOL 追蹤 / 無限 AI，價格 TBD)
  - 響應式設計：mobile-first，靜態頁面無需 client-side data fetching（僅 auth 狀態檢查）
  - 不含：Stripe 結帳流程 (v1.0.0)、法律條款內容 (placeholder 連結)、A/B 測試、Analytics
...
- [x] ~~YouTube 影片逐字稿擷取~~ → **已提前至 v0.1.0 Phase 12a**
- [ ] 多市場支援 (台股、港股、加密貨幣)
- [ ] Dark Mode
- [ ] 付費機制設計 (Phase 14 — Stripe 整合、訂閱管理、定價頁面)
- [ ] 文章熱度統計
- [ ] AI 文章摘要
- [ ] 編輯建議系統 (DB 表已存在於 migration 001)
- [ ] **KOL 平台化 (雙層用戶模型)** — 長期願景
  ...
  - 前置條件：先驗證 v0.1.0 市場需求，確認有足夠用戶基礎後再投入
...
| 2.3  | 2026-03-01 | 大型盤點 (commit b93152b)：A/B 測試框架 + Welcome 預註冊頁面 + 統一錯誤處理 (ApiError + parse-error) + 驗證模組擴充 + DB 原子操作 (migration 013-018) + KOL/Stock 統計 views + 三角標記圖表元件 + PriceChangeBadge + use-seen-posts hook + 6 個單元測試 + 論點 UI 暫停 (ArgumentPlaceholder) |
| 3.0  | 2026-03-07 | Release 01 新增 Phase 12b (KOL Profile 爬取 + 訂閱監控)：3 新表 (kol_sources + kol_subscriptions + scrape_jobs)、YouTubeChannelExtractor (YouTube Data API v3)、profile-scrape.service、Vercel Cron 背景處理、訂閱系統、8 子階段 4 波平行執行；效能策略 Option A+B (即時首批 + 平行批次 + 進度 UI)；Twitter 延後 |
| 3.1  | 2026-03-07 | Phase 12b 擴展：統一 KOL 追蹤額度 (Free 10 / Paid 50，取代舊 5 KOL)；歷史資料擴展 (+10 篇/新訂閱者，上限 200 影片/500 推文/500 貼文)；平台路線圖 YouTube→Twitter (pay-as-you-go)→Instagram+Facebook (Bright Data/Apify)；AI 模型可配置 env var；Phase 12b-9 (Meta 平台) 新增；定價方案更新 |
| 3.2  | 2026-03-08 | Phase 15 (AI 快取層) 移除 — Phase 12b 重複檢查已涵蓋；替換為 Phase 15-lite (AI 模型版本追蹤 + 重新分析)：posts.ai_model_version 欄位 + 重新分析按鈕，為 Argument Analysis 重啟做準備 |
| v0.2.0-dev | 2026-03-08 | 版本命名統一為語義版本 (v0.1.0/v0.2.0/v1.0.0)，取代 MVP/Release 01/Release 02；盤點 v0.2.0 完成項目 (Phase 12b, Phase 17, Twitter oEmbed, RWD 皆已完成)；Phase 15-lite 補完 + Phase 16 (社群洞察) 為 v0.2.0 最後兩個功能；AI 文章摘要 + 編輯建議系統延後至 v1.0.0 |
