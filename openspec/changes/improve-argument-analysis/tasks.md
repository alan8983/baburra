...
- [ ] **1.1** Add `generateStructuredJson()` to `src/infrastructure/api/gemini.client.ts`
...
- [ ] **1.2** Create DB migration `supabase/migrations/027_argument_statement_type.sql`
  - Add `statement_type TEXT DEFAULT 'mixed'` column to `post_arguments`

### Phase 2: Domain Models & Types

- [ ] **2.1** Add `statementType` field to domain types
...
- [ ] **3.1** Define `ARGUMENT_RESPONSE_SCHEMA` constant in `ai.service.ts`
  - JSON Schema for the structured output, including `statementType` enum

- [ ] **3.2** Rewrite `ARGUMENT_EXTRACTION_PROMPT`
...
- [ ] **3.3** Update `buildRevisionPrompt()` to include `statementType` in the expected output format

- [ ] **3.4** Implement `verifyArguments()` function in `ai.service.ts`
...
- [ ] **3.5** Update `extractArguments()` flow
...
- [ ] **4.1** Update `/api/ai/extract-arguments/route.ts`
  - Pass `statementType` through to `createPostArguments()`

- [ ] **4.2** Update `/api/ai/extract-draft-arguments/route.ts`
  - Include `statementType` in draft AI arguments response

- [ ] **4.3** Update `/api/posts/[id]/reanalyze/route.ts`
  - Pass `statementType` through `replacePostArguments()`

- [ ] **4.4** Update quick-input and import pipeline
...
- [ ] **5.2** Update `ArgumentCard` in `src/components/ai/post-arguments.tsx`
  - Show fact/opinion badge for non-mixed arguments
  - Thread `statementType` through `ArgumentItem` type

- [ ] **5.3** Update argument display in `src/components/ai/stock-arguments-tab.tsx`
  - Thread `statementType` through to the argument timeline/cards

### Phase 6: Tests & Docs

- [ ] **6.1** Update/add unit tests in `src/domain/services/__tests__/ai.service.test.ts`
...
- [ ] **6.2** Update `docs/WEB_DEV_PLAN.md` and `docs/BACKLOG.md`
  - Document the argument analysis improvement

- [ ] **6.3** Run `npm run type-check && npm run lint && npm test` — fix any issues
